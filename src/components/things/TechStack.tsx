...
const JustBox = ({ techname, position, xrad, zrad, rotationSpeed, leng}: TechImg) => {
...
const TechBox = ({ techname, position, xrad, zrad, rotationSpeed, leng}: TechImg) => {
  const ref = useRef<THREE.Mesh>(null!);
  const [clicked, setClicked] = useState(false);
  const [hovered, hover] = useState(false);
  // useFrame((_, delta) => {
...
export const RotatingTechs = () => {
  ...
  function randomorbitpos (num) {
    return (Math.max(Math.random()*num, num), Math.max(Math.random()*num, num ), Math.max(Math.random()*num, num ))
  };
  ...
  let i=0;
  const [techs, setTechs] = useState<TechImg[]>([...Array(cubeNumber)].map<TechImg>(() => 
    ({
