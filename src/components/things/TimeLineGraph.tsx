...
const generateCandles = (num, candleHeights) => {
  ...
  const candles = Array.from({ length: numCandles }, (_, i) => {
    ...
    let candleColor = currentPrice >= startingPrice ? '#009655' : 'maroon';
    ...
    return (
      ...
      {milestones.includes(i) && (
        <>
          ...
          <motion.div className="miledivebox milestone-label" animate={{ opacity:1}} initial={{opacity:0}}
...
function TimeLineGraph() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (containerElement!=null) {
        const scrollOffset = 250-containerElement.getBoundingClientRect().top;
        const containerHeight = containerElement.clientHeight;
        
        const nowCandles = scrollOffset/6; //Math.floor(scrollOffset*2 / containerHeight);
  ...
  return (
    <div className='timeline' ref={containerRef} style={{width: widthStack, overflow: 'scroll'}}>
